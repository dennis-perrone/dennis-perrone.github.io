---
layout: post
title:   "Risk Management Framework Series (Part I): Overview"
date:    2020-12-14 08:13:50 -0500
category: tech
tags: [rmf, compliance, information-assurance]
author: Dennis Perrone
excerpt: The first post in a series of posts to outline and serve as an introduction to the Risk Management Framework (RMF).
---

# Series Overview
This post is the first post in a multipart series as an introduction to RMF introduction which defines the roles that an individual can play within the RMF ecosystem.

* Risk Management Framework (RMF) Series:
    * [Part I: Overview](/tech/2020/12/rmf-overview) (**Currently Here**)
    * [Part II: The 6 Steps](/tech/2020/12/rmf-steps)
    * [Part III: Positions and Roles](/tech/2020/12/rmf-positions)

The purpose of this post is to layout an overview of the Risk Management Framework, what it is and the supporting publications that make it what it is. 

# What is Risk Management Framework (RMF)
Risk Management Framework or RMF is the system that the United States federal government utilizes to ensure a cyber security and risk-based compliance throughout the lifecycle of an Information System. It is the byproduct of a few factors. Initially, there was the E-Government Act ([Public Law 107-347](https://www.congress.gov/107/plaws/publ347/PLAW-107publ347.pdf)) which led to the [Federal Information Security Modernization Act](https://www.congress.gov/113/plaws/publ283/PLAW-113publ283.pdf) or FISMA. 

According to NIST, the main purpose of RMF is to manage organizational risk. 

This is accomplished by executing a 6 step process. These processes will be covered in more depth in a future post. For the sake of exposure, those steps are:
1. Categorize
2. Select
3. Implement
4. Assess
5. Authorize
6. Monitor

The publications below should enable a quick reference to the most important publications throughout RMF. For the full list of FIPS and NIST publications, check out the links below.
* [All NIST Special Publications](https://csrc.nist.gov/publications/sp800)
* [All FIPS Publications](https://csrc.nist.gov/publications/fips)

# Important Publications
* Committee on National Security Systems (CNSSI)
    * [CNSSI 1253](https://www.dcsa.mil/portals/91/documents/ctp/nao/CNSSI_No1253.pdf)
* Federal Information Processing Standards (FIPS)
    * [FIPS 199](https://doi.org/10.6028/NIST.FIPS.199)
    * [FIPS 200](https://doi.org/10.6028/NIST.FIPS.200)
* National Institute of Standards and Technology (NIST) Special Publications (SPs)
    * [NIST SP 800-18](https://doi.org/10.6028/NIST.SP.800-18r1)
    * [NIST SP 800-37 Revision 2](https://doi.org/10.6028/NIST.SP.800-37r2)
    * NIST SP 800-53 (Series):
        * [NIST SP 800-53 Revision 4](https://doi.org/10.6028/NIST.SP.800-53r4)
        * [NIST SP 800-53 Revision 5](https://doi.org/10.6028/NIST.SP.800-53r5)
    * [NIST SP 800-47](https://doi.org/10.6028/NIST.SP.800-47)
    * [NIST SP 800-59](https://doi.org/10.6028/NIST.SP.800-59)
    * NIST SP 800-60 (Series):
        * [NIST SP 800-60 Volume 1](https://doi.org/10.6028/NIST.SP.800-60v1r1)
        * [NIST SP 800-60 Volume 2](https://doi.org/10.6028/NIST.SP.800-60v2r1)
    * [NIST SP 800-137](https://doi.org/10.6028/NIST.SP.800-137)
    * [NIST SP 800-137A](https://doi.org/10.6028/NIST.SP.800-137A)
    * NIST SP 800-160 (Series):
        * [NIST SP 800-160 Volume 1](https://doi.org/10.6028/NIST.SP.800-160v1)
        * [NIST SP 800-160 Volume 2](https://doi.org/10.6028/NIST.SP.800-160v2)
* Supporting Instructions and Documents
    * [NIST SP 800-30](https://doi.org/10.6028/NIST.SP.800-30r1)
    * [NIST SP 800-34](https://doi.org/10.6028/NIST.SP.800-34r1)
    * [NIST SP 800-39](https://doi.org/10.6028/NIST.SP.800-39)
    * NIST SP 800-53 Supporting Publications
        * [NIST SP 800-53A Revision 4](https://doi.org/10.6028/NIST.SP.800-53Ar4)
        * [NIST SP 800-53B](https://doi.org/10.6028/NIST.SP.800-53B)
    * [NIST SP 800-53B](https://doi.org/10.6028/NIST.SP.800-53B)
    * [NIST SP 800-61 Revision 2](https://doi.org/10.6028/NIST.SP.800-61r2)
    * [NIST SP 800-128](https://doi.org/10.6028/NIST.SP.800-128)
